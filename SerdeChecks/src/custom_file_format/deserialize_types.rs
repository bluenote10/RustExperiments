...
fn parse_track(time_quantization: u32, input: &[u8]) -> IResult<&[u8], Track> {
    let (input, name) = parse_string(input)?;
    let (input, is_percussion) = parse_bool(input)?;
    let (input, tuning) = parse_tuning(input)?;
    let (input, notes) = parse_vector(|input| parse_note(time_quantization, input), input)?;
    Ok((
        input,
        Track {
            name,
...
fn parse_note(time_quantization: u32, input: &[u8]) -> IResult<&[u8], Note> {
